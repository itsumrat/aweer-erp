...
@section('title', 'Dashboard - Transfer Edit')
                                                ...
                                                <div class="row">
                                                    <div class="col-md-2">
                                                        <div class="form-group">
                                                            <label>Date</label>
                                                            <input id="datetimepicker" type="text" class="form-control" name="date" value="{{ $transfer->date }}">
                                                        </div>
                                                    </div>
                                                    <div class="col-md-2">
                                                        <div class="form-group">
                                                    ...
                                                    <div class="col-md-3">
                                                        <div class="form-group">
                                                            ...
                                                            <select class="form-control" name="status">
                                                                <option {{ $transfer->status == 1?'selected':'' }} value="1">Sent</option>
                                                                <option {{ $transfer->status == 2?'selected':'' }} value="2">Received</option>
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-4">
                                        ...
                                        <div class="col-12 mt-1">
                                            <div style="padding: 1rem;background-color: #f1f1f1">
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <label>Note</label>
                                                        <textarea class="form-control" rows="3" name="note">{{ $transfer->note }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
const app = new Vue({
    ...
    data: {
        ...
        location_list : [],
        selected_location: {
            name : '{{ $transfer->transfer_to_location->name }}',
            id: '{{ $transfer->transfer_to_location->id }}',
        },

        selected_location_edit: {
            label : '{{ $transfer->transfer_to_location->name }}',
            code: '{{ $transfer->transfer_to_location->id }}',
        },
    ...
    methods:{
        ...
        autoGenerateRef: function(){
            let ref = this;
            if (ref.reference == '') {
            ref.reference =  'TRN'+Date.now();
            }

        },
        edit_selected_transfer: function(){
            let ref = this;
            let transfer_id = ref.selected_transfer_id;
            axios.post('/api/transfer/items/by/id', {'id': transfer_id}).then(function(resposne){
                let transfer_items = resposne.data.transfer_items;
                ref.selected_items = [];
                for (var i = 0; i < transfer_items.length; i++) {

                   transfer_items[i].item.quantity = transfer_items[i].quantity;
                    ref.selected_items.push(transfer_items[i].item);
                }
