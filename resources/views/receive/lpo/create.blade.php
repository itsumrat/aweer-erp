                                                ...
                                                <div class="row">
                                                    <div class="col-md-3">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text" class="form-control" v-model="product_code_in">
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3">
                                                        <div class="form-group">
                                                    ...
                                                    <div class="col-md-3">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text" class="form-control" v-model="selected_item.quantity" readonly>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3">
                                                        <div class="form-group">
                                                            <label>Discount</label>
                                                            <input type="text" class="form-control" v-model="selected_item.discount" readonly>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3">
                                                        <div class="form-group">
                                                    ...
                                                    <table class="table table-striped table-bordered" style="width:100%">
                                                        ...
                                                        <tbody v-if="selected_items.length > 0">
                                                            <tr v-for="(selected_item, index) in selected_items">
                                                                ...
                                                                @{{ selected_item.products_barcode.code + '-' + selected_item.products_barcode.name}}</td>
                                                                <td>@{{ selected_item.cost }}</td>
                                                                <td><input type="number" class="form-control" :value="selected_item.cost" v-bind:name="getInputName(index, 'cost')" readonly ></td>
                                                                <td><input type="number" class="form-control" :value="selected_item.quantity" v-bind:name="getInputName(index, 'quantity')" readonly ></td>
                                                                <td><input type="number" class="form-control" :value="selected_item.discount" v-bind:name="getInputName(index, 'discount')" readonly></td>
                                                                <td>@{{ (selected_items[index].quantity * selected_item.cost) - (((selected_items[index].quantity * selected_item.cost)*selected_items[index].discount)/100) }}</td>
...
const app = new Vue({
    ...
    methods:{
        fetchItems: function(search, loading){
            ...
            if(search != ''){
            axios.post("{{ url('/api/item/list/receive') }}",{'search':search, 'purchase_id': ref.purchase_id}).then(function(response){
        ...
        fetch_item_info: function(value){
            ...
            axios.post("{{ url('/api/item/info/with/purchase') }}", {'id':product_id, 'purchase_id': ref.purchase_id}).then(function(response){
                ...
                // ref.countTotals();
                ref.product_code_in = data.products_barcode ? data.products_barcode.code : 0000;
                ref.selected_item.unit_cost = data.cost;
        ...
        countTotals(){
            ...
            for (var i = 0; i < countable_list.length; i++) {
                if(!counted_elements.includes(countable_list[i].products_barcode.code )){
                    counted_elements.push(countable_list[i].products_barcode.code);
                ...
                tot = parseFloat(ref.grand_total) + (parseInt(countable_list[i].quantity) * parseFloat(countable_list[i].cost));

                tot = tot - (tot * parseFloat(countable_list[i].discount))/100;
